...
UNIFEX_TERM scale(UnifexEnv * env, UnifexPayload * payload, State * state) {
  ...
  if (av_image_copy_to_buffer(
      frame -> data, payload_size, (const uint8_t *
        const * ) state -> output_data,
